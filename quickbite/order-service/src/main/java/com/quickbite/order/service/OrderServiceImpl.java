...
import com.quickbite.order.producer.SagaProducer;
...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderServiceImpl(OrderRepository orderRepository, NotificationProducer notificationProducer, SagaProducer sagaProducer, RestaurantClient restaurantClient) {
    ...
    public OrderResponse placeOrder(Long customerId, OrderRequest request) {
        ...
        try {
            ...
            notificationProducer.sendNotification(customerNote);
        } catch (Exception e) {
            System.err.println(">>> Failed to send order placement notifications: " + e.getMessage());
        }

        // Publish OrderCreatedEvent for Saga
        Map<String, Object> orderData = new HashMap<>();
        orderData.put("orderId", savedOrder.getId());
        orderData.put("customerId", savedOrder.getCustomerId());
        orderData.put("restaurantId", savedOrder.getRestaurantId());
        orderData.put("totalAmount", savedOrder.getTotalAmount());
        
        sagaProducer.publishOrderCreated(orderData);
    ...
    public OrderResponse updateOrderStatus(Long orderId, OrderStatus status) {
        ...
        try {
            ...
            notificationProducer.sendNotification(note);
        } catch (Exception e) {
